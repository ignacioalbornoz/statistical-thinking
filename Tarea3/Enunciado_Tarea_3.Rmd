...
```{r}
# Cargar los datos (asegúrate de ajustar la ruta si es necesario)
...
# Paso 1: Separar el dataset en fumadores y no fumadores
...
# Paso 2: Función para calcular correlación manualmente
...
# Paso 3: Encontrar la variable con mayor correlación con 'charges' para fumadores
...
# Paso 4: Encontrar la variable con mayor correlación con 'charges' para no fumadores
...
# Paso 5: Implementar la regresión lineal para ambos grupos con la variable más correlacionada
...
# Paso 1: Calcular la correlación de cada variable numérica con 'charges' para fumadores
correlations_smokers <- sapply(variables_numericas, function(var) calc_correlation(smokers[[var]], smokers$charges))
top_two_vars_smokers <- names(sort(abs(correlations_smokers), decreasing = TRUE))[1:2]

# Paso 2: Calcular la correlación de cada variable numérica con 'charges' para no fumadores
correlations_non_smokers <- sapply(variables_numericas, function(var) calc_correlation(non_smokers[[var]], non_smokers$charges))
...
# Paso 3: Implementar la regresión lineal multivariable desde cero
